...
| `APP_CATALOGO_INTERNAL_API_KEY` | Required visible-ASCII secret shared only with the SuperApp BFF to authenticate its pseudonymous public-catalog client identity |
| `APP_CATALOGO_INTERNAL_REQUEST_MAX_SKEW` | Bounded acceptance window for signed BFF public-catalog request timestamps |
...
| `GEMINI_API_KEY` | Server-side Gemini key used for query/document embeddings, HyDE, and grounded result summaries; local shells can provide it through `~/.zshrc` |
...
| `SEARCH_EXACT_WEIGHT` / `SEARCH_FULL_TEXT_WEIGHT` / `SEARCH_TRIGRAM_WEIGHT` / `SEARCH_SEMANTIC_WEIGHT` / `SEARCH_HYDE_WEIGHT` | Versioned weighted-RRF configuration owned entirely by app-catalogo |
| `SEARCH_SUMMARY_TIMEOUT` / `SEARCH_SUMMARY_MAX_CONCURRENCY` / `CACHE_SEARCH_SUMMARY_TTL` | Deadline, process-wide capacity bound, and generated-summary cache lifetime for grounded search summaries |
...
just test-integration # full suite with isolated PostgreSQL and Redis containers
...
GET  /api/public/service-categories/:category/subcategories
GET  /api/public/services        paginated service browse with category filters
GET  /api/public/services/:slug  canonical public service detail; historical slugs redirect permanently
POST /api/public/suggest         strict, bounded service typeahead without query strings
GET  /api/public/services/:slug/relations  curated journey, taxonomy, and semantic relations in one catalog snapshot
POST /api/public/search-summary  revision-bound Gemini summary with server-allowlisted citations
```

Search keeps the canonical user query separate from synonym expansion. Exact matches, Portuguese full-text matches, title trigram matches, and version-compatible semantic vectors form independent app-catalogo-owned pools. Every retriever keeps only its best signal per canonical entity before applying its candidate limit, so aliases cannot consume multiple fusion positions and signals found through different aliases accumulate on the same entity. ANN retrieval first overfetches a bounded alias window and then canonicalizes it; this mitigates alias saturation without claiming exhaustive ANN recall at the configured resource bound. Weighted reciprocal-rank fusion produces one deterministic global order; an optional cross-encoder may reorder a complete leading window, and pagination runs last. If Gemini, HyDE, or the reranker fails, the pipeline degrades to the strongest available stage without returning a partial external response.
...
Every response includes `search_id`, `catalog_revision`, `effective_pipeline`, `degraded`, the complete non-secret `ranker_descriptor`, and a fingerprinted `ranker_version`. The descriptor versions retrieval SQL semantics, query expansion, weights, semantic compatibility and overfetch, HyDE generation, and reranker availability. It contains only catalog-owned ranking stages; retired external retriever diagnostics and weights are not part of the contract. `catalog_revision` uses `catalog-v2:<content-revision>:window-until-<unix-microseconds|infinity>`: it changes both after catalog writes and when the PostgreSQL eligibility clock crosses the next `valid_from` or `valid_until` boundary. Ranked cache keys include that exact results snapshot but exclude pagination; a cache entry contains the complete final order, facets, provenance, and public items so every page is cut from one ranking without re-running Gemini or the reranker. Concurrent misses for the same ranking are coalesced into bounded shared work. An oversized complete snapshot is served as a bounded page without being cached, while a page that itself exceeds the public response byte limit fails explicitly and is never truncated. Every apparent hit is revalidated before return, and writes use the shorter of the configured TTL and the remaining database-observed eligibility window. A boundary crossed during execution marks the response degraded and prevents caching. Runtime fallbacks are also returned with `degraded: true` and are never cached under the nominal ranker identity. Search metrics expose cache outcomes, fallbacks, candidate counts, reranker outcomes, zero-result searches, and end-to-end latency without using raw queries as metric labels.
...
Service typeahead is executed directly against active, currently eligible catalog rows. Ranking is accent-insensitive and deterministic: exact and prefix title matches lead word-prefix, title-substring, summary, and Portuguese full-text matches, followed by trigram similarity, shorter title, binary title order, and catalog ID. The endpoint accepts only a bounded JSON body, returns only canonical service slugs, and constructs the same category route consumed by the SuperApp.

Service relations are returned from one repeatable-read catalog snapshot. Curated citizen-journey edges lead the recommendation blend, followed by subcategory, category, and version-compatible embedding proximity. The thematic cluster is the current catalog category rather than an opaque offline cluster assignment. Missing, inactive, future, expired, or deleted targets are excluded at read time, and historical source slugs redirect to their canonical relation endpoint.

Search summaries accept only a catalog revision and an ordered, unique list of catalog UUIDs. The service rehydrates every candidate from that exact public eligibility window before any provider call, rejects stale revisions, and sends Gemini only the query plus bounded title/summary projections as untrusted JSON data. Gemini returns text and candidate indexes, never identifiers or URLs; the service maps indexes back to trusted catalog routes. Calls are coalesced, concurrency- and deadline-bounded, and cached by query, revision, and candidate order. A missing key, provider error, invalid structured response, or exhausted capacity degrades to `generated: false` with no fabricated content.

Privacy boundary: on a semantic cache miss, when Gemini retrieval is configured, the query is sent server-side to Google Gemini to produce its query embedding. Grounded summaries also send the normalized query and bounded public title/summary projections for the explicitly selected catalog candidates. When HyDE is enabled and weighted, the same query is serialized as untrusted JSON user data under a fixed system instruction, the hypothetical document is generated with fixed generation settings, and that document is embedded. Gemini seed determinism is best effort; the prompt version, prompt SHA-256, model, generation settings, and determinism policy are therefore included in `ranker_descriptor` and its fingerprint. When reranking is enabled, the normalized query and the bounded public title/description projection of the leading candidates are sent to the configured `RERANKER_URL`. Treat these endpoints as trusted data processors: redirects are rejected, payloads and response bodies are excluded from errors, and production traffic must use HTTPS outside loopback development. Raw query text is excluded from application logs and metric labels. Every frontend must disclose this provider processing before submission and warn users not to enter personal or sensitive information.
...
Run `just test-integration` to reproduce the complete CI test environment locally. The command starts dedicated PostgreSQL with pgvector and Redis containers on ephemeral loopback ports, applies migrations, enables every database and Redis integration test, and removes its containers and volumes when it exits. It does not reuse the development Compose project or its persistent data.
